...
<body class="bg-slate-800">
    <div class="flex h-full w-full flex-col items-center space-y-5">
        ...
        <!-- Head -->
        <div class="mt-[10%] flex w-[70%] justify-between">
            <h1 class="text-lg font-bold text-white">Tugas Anda</h1>
            <div class="flex items-center space-x-3">
                ...
                <button type="button" onclick="toggleAddTaskModal()" class="rounded-md bg-blue-600 px-2 py-1 font-semibold text-white shadow-md duration-500 active:scale-90 hover:bg-blue-700 opacity">Tambah Tugas</button>
        ...
        <!-- Body -->
        <div class="flex max-h-[60%] w-[70%] flex-col overflow-y-auto">
            <?php 
                if (mysqli_num_rows($getTasks) > 0) {
                while ($row = mysqli_fetch_array($getTasks)): 
            ?>
                <form action="includes/hapus_tugas.php?id=<?= $row['ID'] ?>" method="post" class="my-2 flex w-full justify-between rounded-md bg-slate-700 pl-3 text-white duration-500 hover:scale-100 <?= $row['is_pin'] == 1 ? 'order-first' : '' ?>">
    ...
    <!-- Modal View Task -->
    <div id="viewTask" class="absolute left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 hidden">
        <form action="includes/edit_tugas.php" method="post" class="divide-y w-1/3 max-h-[85%] rounded-md bg-slate-700 px-3 py-3">
            <div class="my-3">
            ...
            <div class="flex w-full items-center justify-between space-x-3">
                ...
                <div class="m-0 flex space-x-3">
                    <button id="btnEdit" name="edit_data" onclick="return confirm('Anda yakin mau mengedit tugas ini?')" disabled class="my-2 rounded bg-yellow-500 px-4 py-1 font-semibold text-white duration-300 active:scale-90 hover:bg-yellow-600">Edit</button>
    ...
    <div id="addTask" class="absolute left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="divide-y w-1/3 max-h-[85%] rounded-md bg-slate-700 px-3 py-3">
            <div class="py-3">
    ...
    <div id="modalStatistik" class="absolute left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="divide-y w-2/3 max-h-[85%] rounded-md bg-slate-700 px-3 py-3">
            <div class="py-3">
